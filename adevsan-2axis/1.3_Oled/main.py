...
if ('A_ABS.txt' in os.listdir()):
    with open('A_ABS.txt') as ENC_A_file:
        a = ENC_A_file.read()
        ENC_A_file.close()
        abs_A_angle = int(a)
        inc_A_angle = int(a)
...
if ('B_ABS.txt' in os.listdir()):
    with open('B_ABS.txt') as ENC_B_file:
        b = ENC_B_file.read()
        ENC_B_file.close()
        abs_B_angle = int(b)
        inc_B_angle = int(b)
...
def read_Enc_A_Value():
    global Enc_A_Value
    Enc_A_Value = i2c.readfrom_mem(AS5600_id,0x0c,2)
    Enc_A_Value = ((Enc_A_Value[0] & 15) << 8) | Enc_A_Value[1]
    return Enc_A_Value

def read_Enc_B_Value():
    global Enc_B_Value
    Enc_B_Value = i2c_1.readfrom_mem(AS5600_id,0x0c,2)
    Enc_B_Value = ((Enc_B_Value[0] & 15) << 8) | Enc_B_Value[1]
    return Enc_B_Value
